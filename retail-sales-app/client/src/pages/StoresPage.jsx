...
export default function StoresPage() {
  ...
  function load() {
    setLoading(true);
    api.get(`/stores?dataset=${dataset}&includeInactive=true`).then(setStores).finally(() => setLoading(false));
  }
  ...
  async function addStore(e) {
    ...
    setAdding(true);
    try {
      await api.post('/stores', { dataset, name: newName.trim(), code: newCode.trim() || null });
      setNewName('');
      setNewCode('');
      load();
    } catch (err) {
  ...
  return (
    <div>
      <div className="card">
        <h2>Stores</h2>
        <p className="text-muted">Manage stores for the <strong>{dataset === 'company' ? 'Company Owned' : 'Franchise'}</strong> dataset. Deactivate a store when it closes instead of deleting it, so its sales history is kept.</p>
